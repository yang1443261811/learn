...
class WebSocket
{
    ...
    public function __construct($host, $port)
    {
        ...
        $this->debug(["server: {$this->master} started,pid: {123}"]);
        while (true) {
            try {
                $this->doServer();
            } catch (\Exception $e) {
    ...
    private function doServer()
    {
        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                //如果是一个多个连接的队列,只会处理第一个,如果没有连接的话,进程将会被阻塞,直到连接上.如果用set_socket_blocking或socket_set_noblock()设置了阻塞,
    ...
    public function handShake($socket, $buffer)
    {
        ...
        $hash = base64_encode(sha1($key . "258EAFA5-E914-47DA-95CA-C5AB0DC85B11", true));// 升级key的算法
...
$ws = new WebSocket("127.0.0.1", "9999");